...
--------------- Files
makefolder("SkullHub")
makefolder('SkullHub\\Anime Adventures')


local DefaultFiles = {

	['SkullHub\\Settings_' .. player.Name] = {
...
local Settings = HttpService:JSONDecode(readfile('SkullHub\\Settings_' .. player.Name)) 

local function Save (valueName, newValue)
	Settings[valueName] = newValue
	writefile('SkullHub\\Settings_' .. player.Name, HttpService:JSONEncode(Settings))
end

local function GetSave (valueName)
	local value = Settings[valueName]
	if value == nil then
		if DefaultFiles['SkullHub\\Settings_' .. player.Name][valueName] ~= nil then
			Save(valueName, DefaultFiles['SkullHub\\Settings_' .. player.Name][valueName])
		else
...
DiscordLabel.Text = "Discord: NxHT2rSynR"
...
function getMacroUnits(macroName, textBlocks)
	...
	local macroExist = isfile('SkullHub\\Anime Adventures\\' .. nameMacro)
	local macroExistjson = isfile('SkullHub\\Anime Adventures\\' .. nameMacro .. '.json')
	...
	if macroExist or macroExistjson then
		...
		local sucess, response = pcall(function()
			local MacroAbout = HttpService:JSONDecode( readfile( 'SkullHub\\Anime Adventures\\' .. nameMacro) )
...
for tabName, mapsList in pairs(macroMapList) do
	...
	for _, mapName in ipairs(mapsList) do
		...
		newDDLMacroMap.MouseButton1Click:Connect(function()
			...
			local MacrosList = listfiles('SkullHub\\Anime Adventures')
...
local function RecordMacroFunc ()
	...
	local macroExist = isfile('SkullHub\\Anime Adventures\\' .. MacroFile )
	local macroExistJson = isfile('SkullHub\\Anime Adventures\\' .. MacroFile .. '.json')

	if macroExistJson then MacroFile = 'SkullHub\\Anime Adventures\\' .. MacroFile .. '.json'
	elseif macroExist then MacroFile = 'SkullHub\\Anime Adventures\\' .. MacroFile
...
createMacroBox.FocusLost:Connect(function(enterPressed)
	...
	writefile('SkullHub\\Anime Adventures\\' .. createMacroBox.Text .. '.json', HttpService:JSONEncode({}))
end)
...
deleteMacroButton.MouseButton1Click:Connect(function()
	...
	local macroExist = isfile('SkullHub\\Anime Adventures\\' .. item)
	local macroExistjson = isfile('SkullHub\\Anime Adventures\\' .. item .. '.json')
	if macroExistjson then item = item .. '.json' end

	if not macroExist and not macroExistjson then return end

	delfile('SkullHub\\Anime Adventures\\' .. item)
...
equipMacroUnitsButton.MouseButton1Click:Connect(function()
	...
	local macroExist = isfile('SkullHub\\Anime Adventures\\' .. macroSelected)
	local macroExistjson = isfile('SkullHub\\Anime Adventures\\' .. macroSelected .. '.json')
	...
	local MacroAbout = HttpService:JSONDecode( readfile( 'SkullHub\\Anime Adventures\\' .. macroSelected) )
...
function HideTabFunc (enabled)
	...
	local userNameNew = 'SkullHub ON TOP'
	...
	local data = {
		...
		["embeds"] = {

			{
				...
				['footer'] = {
					['text'] = string.format("by hungquan99 (%s)", os.date("%X")), 
				},
				['fields'] = {
					{
						['name'] = "Player Stats",
